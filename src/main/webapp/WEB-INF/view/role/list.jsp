...
<div class="page-container">
    <span class="l" id="toolbar">
        <shiro:hasPermission name='role:delete'><a href="javascript:;" onclick="multiDel()" class="btn btn-danger radius">
            <i class="Hui-iconfont">&#xe6e2;
            </i> 批量删除
        </a></shiro:hasPermission>
        <shiro:hasPermission name='role:insert'><a href="javascript:;" onclick="role_add('添加角色','role/add','700','500')" class="btn btn-primary radius">
            <i class="Hui-iconfont">&#xe600;
            </i> 添加角色
        </a></shiro:hasPermission>
    </span>
...
<script type="text/javascript">
    $('#roleTable').bootstrapTable({
        ...
        smartDisplay:false,
        showColumns:true,
        url:"role/getRoles", //从 url 获取 json 数据
        /*
			ajax请求以后回调函数的处理
    ...
    */
    function operationFormatter(value) {
        var html ="<shiro:hasPermission name='role:update'><span onclick='role_edit("+value+")' style='cursor: pointer;' class='glyphicon glyphicon-pencil'></span></shiro:hasPermission>&nbsp;&nbsp";
        html+="<shiro:hasPermission name='role:delete'><span onclick='role_del(this,"+value+")'  style='cursor: pointer;color:red' class='glyphicon glyphicon-trash'></span></shiro:hasPermission>";
        return html;
    }
    ...
    function multiDel() {
        let ids = $.map($('#roleTable').bootstrapTable('getSelections'), function (row) {
            return row.roleId;
        })
        if (ids.length == 0) {
            //弹出一个提示消息
            layer.msg('没有选中任何角色', {time: 1000, icon:5});
        } else {
    ...
    */
    ...
    function role_edit(roleId){
        layer_show("修改用户","role/edit?roleId="+roleId,700,500);
    }
