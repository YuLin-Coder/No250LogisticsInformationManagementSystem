...
<div class="page-container">
    <span class="l" id="toolbar">
        <shiro:hasPermission name='permission:delete'><a href="javascript:;" onclick="multiDel()" class="btn btn-danger radius">
            <i class="Hui-iconfont">&#xe6e2;
            </i> 批量删除
        </a></shiro:hasPermission>
        <shiro:hasPermission name="permission:insert"><a href="javascript:;" onclick="permission_add('添加权限','permission/add','700','500')" class="btn btn-primary radius">
            <i class="Hui-iconfont">&#xe600;
            </i> 添加权限
        </a></shiro:hasPermission>
    </span>
...
<script type="text/javascript">
		...
		*/
        ...
        columns: [ //表格显示数据对应的表头设置，
            ...
            */
            ...
            {field: 'parent.name',title: '父权限'},
            {field: 'type',title: '权限类型',formatter:typeNameFormatter},
            /*
                操作列的设置（删除，修改）
    ...
    */
    function operationFormatter(value) {
        var html ="<shiro:hasPermission name='permission:update'><span onclick='permission_edit("+value+")' style='cursor: pointer;' class='glyphicon glyphicon-pencil'></span></shiro:hasPermission>&nbsp;&nbsp";
        html+="<shiro:hasPermission name='permission:delete'><span onclick='permission_del(this,"+value+")'  style='cursor: pointer;color:red' class='glyphicon glyphicon-trash'></span></shiro:hasPermission>";
        return html;
    }
    
    function typeNameFormatter(value) {
        if (value == 'menu') {
    ...
    function multiDel() {
        ...
        if (ids.length == 0) {
            //弹出一个提示消息
            layer.msg('没有选中任何权限', {time: 1000, icon:5});
        } else {
    ...
    */
    ...
    function permission_edit(permissionId){
        layer_show("修改权限","permission/edit?permissionId="+permissionId,700,500);
    }
