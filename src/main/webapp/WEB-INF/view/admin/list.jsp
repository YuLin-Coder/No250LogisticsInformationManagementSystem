...
<div class="page-container">
    <span class="l" id="toolbar">
        <shiro:hasPermission name="admin:delete"><a href="javascript:;" onclick="multiDel()" class="btn btn-danger radius">
            <i class="Hui-iconfont">&#xe6e2;
            </i> 批量删除
        </a></shiro:hasPermission>
        <shiro:hasPermission name="admin:insert"><a href="javascript:;" onclick="admin_add('添加管理员','admin/add','700','500')" class="btn btn-primary radius">
            <i class="Hui-iconfont">&#xe600;
            </i> 添加管理员
        </a></shiro:hasPermission>
    </span>
...
<script type="text/javascript">
    $('#adminTable').bootstrapTable({
        ...
        smartDisplay:false,
        showColumns:true,
        url:"admin/getAdmins", //从 url 获取 json 数据
        /*
			ajax请求以后回调函数的处理
    ...
    */
    function operationFormatter(value) {
        var html ="<shiro:hasPermission name='admin:update'><span onclick='admin_edit("+value+")' style='cursor: pointer;' class='glyphicon glyphicon-pencil'></span></shiro:hasPermission>&nbsp;&nbsp";
        html+="<shiro:hasPermission name='admin:delete'><span onclick='admin_del(this,"+value+")'  style='cursor: pointer;color:red' class='glyphicon glyphicon-trash'></span></shiro:hasPermission>";
        return html;
    }
    ...
    function multiDel() {
        let ids = $.map($('#adminTable').bootstrapTable('getSelections'), function (row) {
            return row.userId;
        })
        if (ids.length == 0) {
            //弹出一个提示消息
            layer.msg('没有选中任何用户', {time: 1000, icon:5});
        } else {
    ...
    */
    ...
    function admin_edit(userId){
        layer_show("修改用户","admin/edit?userId="+userId,700,500);
    }
