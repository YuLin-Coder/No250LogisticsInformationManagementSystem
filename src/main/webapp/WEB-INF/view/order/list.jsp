...
<div class="page-container">
    <span class="l" id="toolbar">
        <shiro:hasPermission name='order:delete'><a href="javascript:;" onclick="multiDel()" class="btn btn-danger radius">
            <i class="Hui-iconfont">&#xe6e2;
            </i> 批量删除
        </a></shiro:hasPermission>
        <shiro:hasPermission name="order:insert"><a href="javascript:;" onclick="order_add('添加订单','order/add','1200','700')" class="btn btn-primary radius">
            <i class="Hui-iconfont">&#xe600;
            </i> 添加订单
        </a></shiro:hasPermission>
    </span>
...
<script type="text/javascript">
    $('#orderTable').bootstrapTable({
        ...
        smartDisplay:false,
        showColumns:true,
        url:"order/getOrders", //从 url 获取 json 数据
        /*
			ajax请求以后回调函数的处理
    ...
    */
    function operationFormatter(value) {
        var html ="<shiro:hasPermission name='order:update'><span onclick='order_edit("+value+")' style='cursor: pointer;' class='glyphicon glyphicon-pencil'></span></shiro:hasPermission>&nbsp;&nbsp";
        html+="<shiro:hasPermission name='order:delete'><span onclick='order_del(this,"+value+")'  style='cursor: pointer;color:red' class='glyphicon glyphicon-trash'></span></shiro:hasPermission>";
        return html;
    }
    ...
    function multiDel() {
        let ids = $.map($('#orderTable').bootstrapTable('getSelections'), function (row) {
            return row.orderId;
        })
        if (ids.length == 0) {
            //弹出一个提示消息
            layer.msg('没有选中任何订单', {time: 1000, icon:5});
        } else {
    ...
    */
    ...
    function order_edit(orderId){
        layer_show("修改订单","order/edit?orderId="+orderId,1200,700);
    }
