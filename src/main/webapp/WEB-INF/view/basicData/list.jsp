...
<div class="page-container">
    <span class="l" id="toolbar">
        <shiro:hasPermission name="basicData:delete"><a href="javascript:;" onclick="multiDel()" class="btn btn-danger radius">
            <i class="Hui-iconfont">&#xe6e2;
            </i> 批量删除
        </a></shiro:hasPermission>
        <shiro:hasPermission name='basicData:insert'><a href="javascript:;" onclick="basicData_add('添加数据','basicData/add','700','500')" class="btn btn-primary radius">
            <i class="Hui-iconfont">&#xe600;
            </i> 添加数据
        </a></shiro:hasPermission>
    </span>
    <%--用 boostrap 生成--%>
    <table id="basicDataTable"></table>
</div>
<!--_footer 作为公共模版分离出去-->
...
<script type="text/javascript">
    $('#basicDataTable').bootstrapTable({
        ...
        smartDisplay:false,
        showColumns:true,
        url:"basicData/getBasicDatas", //从 url 获取 json 数据
        /*
			ajax请求以后回调函数的处理
    ...
    */
    function operationFormatter(value) {
        var html ="<shiro:hasPermission name='basicData:update'><span onclick='basicData_edit("+value+")' style='cursor: pointer;' class='glyphicon glyphicon-pencil'></span></shiro:hasPermission>&nbsp;&nbsp";
        html+="<shiro:hasPermission name='basicData:delete'><span onclick='basicData_del(this,"+value+")'  style='cursor: pointer;color:red' class='glyphicon glyphicon-trash'></span></shiro:hasPermission>";
        return html;
    }
    ...
    function refreshTable() {
        $('#basicDataTable').bootstrapTable('refresh');
    }

    function multiDel() {
        let ids = $.map($('#basicDataTable').bootstrapTable('getSelections'), function (row) {
            return row.baseId;
        })
        if (ids.length == 0) {
            //弹出一个提示消息
            layer.msg('没有选中任何数据', {time: 1000, icon:5});
        } else {
            layer.confirm('确认要删除吗？',function(index){
                $.post("basicData/delete",{
                    basicDataIds: ids.join()
                }).then(function (res) {
                    if (res.status == 'true'){
                        //关闭询问框
                        layer.close(index);
    ...
    */
    /*管理员-增加*/
    function basicData_add(title,url,w,h){
        layer_show(title,url,w,h);
    }
    /*管理员-删除*/
    function basicData_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.post("basicData/delete",{
                basicDataIds: id
            }).then(function (res) {
    ...
    /*管理员-编辑*/
    function basicData_edit(basicDataId){
        layer_show("修改数据","basicData/edit?basicDataId="+basicDataId,700,500);
    }
